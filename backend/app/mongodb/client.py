...
class MongoClientWrapper(Generic[T]):
    ...
    def __init__(
        ...
        if mongo_config is None:
            raise ValueError("MongoDB configuration not found in config file")
            
        self.database_name = database_name or mongo_config.database
        self.mongodb_uri = mongodb_uri or mongo_config.connection_string
    ...
    def insert_one(self, document: T) -> str:
    ...
    def insert_many(self, documents: list[T]) -> list[str]:
        ...
        """
        try:
            doc_dicts = [doc.model_dump(exclude={"id"}) for doc in documents]
    ...
    def find_one(self, filter_dict: dict) -> Optional[T]:
        ...
        Returns:
            Optional[T]: The matching document as a Pydantic model instance
        """
        try:
    ...
    def find_many(self, filter_dict: dict = None, limit: int = None) -> list[T]:
        ...
        Returns:
            list[T]: List of matching documents as Pydantic model instances
        """
        try:
            ...
            for doc in cursor:
                doc["id"] = str(doc.pop("_id"))
                documents.append(self.model.model_validate(doc))
    ...
    def create_index(self, index_spec, **kwargs):
        ...
        """
        try:
            result = self.collection.create_index(index_spec, **kwargs)
